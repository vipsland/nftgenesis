...
export default function MintNONMPPageAirdrop() {
  ...
  const [
    maxNONMPAmountPerAccPerTransaction,
    setMaxNONMPAmountPerAccPerTransaction
  ] = useState(0)
  ...
  useEffect(() => {
    const init = async () => {
      ...
      setMaxNONMPAmountPerAccPerTransaction(
        await getMaxNONMPAmountPerAccPerTransaction(MAIN_STAGE)
      )
    }
  ...
  const incrementPRTAmount = () => {
    if (
      prtAmount < maxNONMPAmountPerAccPerTransaction &&
      prtAmount < remainingNONMP
    ) {
  ...
  const mintNONMPForAIRDROPHandler = async () => {
    ...
    const { success, status: message } = await mintNONMPForAIRDROP({
      prtAmount,
      wallet,
  ...
  return (
    <div className="flex flex-col items-center">
      <img
        width="250"
        height="38"
        src="https://0.m01d.com/x/vipsland.com/c/u/_4/XK/FVeo-o/SGZuFwfXD3RHPCt8ze9XzPkAfgVnX3MTNjzAxI_NP5s/vipsland.webp"
        alt=""
      />
          ...
          <div className="flex flex-col md:flex-row md:space-x-14 w-full mt-10 md:mt-14">
            <div className="relative w-full">
              <div className="font-default z-10 absolute top-2 left-2 opacity-80 filter backdrop-blur-lg text-base px-4 py-2 bg-black border border-brand-black rounded-md flex items-center justify-center text-white font-semibold">
                <p>
                  <span className="text-brand-blue">{totalMintedNONMP}</span> /{" "}
                  {maxSupplyNONMP}
                </p>
              </div>
            ...
            <div className="flex flex-col items-center w-full px-4 mt-16 md:mt-0">
              {wallet ? (
                <div className="font-default flex items-center justify-between w-full">
                  <button
              ...
              <p className="text-sm text-white mt-3">
                {isMintNONMP && wallet ? (
                  <>
                    Remaining Normal Pass: {remainingNONMP}
                    <br />
