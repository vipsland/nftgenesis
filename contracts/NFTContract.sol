...
contract NFTContract is ERC1155, Ownable, ReentrancyGuard {
  ...
  function mintWithRandomID() public {
    ...
    require(
      counterTokenID >= 0 && counterTokenID < NUM_TOTAL * 10,
      'Error: exceeded max supply 10000'
    );

    uint256 tokenID = raffle();

    require(
      tokenID >= 5 && tokenID <= MAX_SUPPLY_FOR_TOKEN,
      'Error: tokenID < 5 OR tokenID > MAX_SUPPLY_FOR_TOKEN'
    );
